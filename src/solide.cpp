...
void Solide::Init(const char* s1, const bool& rep, const int& numrep, const double& rho){ //Pour gmsh
  ...
  for(std::vector<Face>::iterator F=faces.begin();F!=faces.end();F++){
	...
	throw std::invalid_argument( "Masse nulle" );
      ...
      F->masses[0]= sqrt(pow(F->S * (F->centre - solide[voisin1].x0) * F->normale / 3. * rho, 2.));
      F->masses[1] = sqrt(pow(F->S * (F->centre - solide[voisin2].x0) * F->normale / 3. * rho, 2.));
...
void Solide::stresses_bis_MEMM(const double& theta, const double& t, const double& T){
  for(std::vector<Particule>::iterator P=solide.begin();P!=solide.end();P++){ //Calcul des déformations et des contraintes
	...
	if(P->id == faces[f].voisins[0])
	  ...
	  voi = 1;
	Matrix Dij_n(tens(0.5 * (faces[f].Dx[voi] + faces[f].Dx_prev[voi]),  nIJ));
	Matrix Dij_n_sym(tens_sym(0.5 * (faces[f].Dx[voi] + faces[f].Dx_prev[voi]),  nIJ));
	P->discrete_gradient += faces[f].S /  P->V * Dij_n;
	P->discrete_sym_gradient += faces[f].S /  P->V * Dij_n_sym;
      }
    }
...
const double Solide::Energie_potentielle(){
  ...
  for(std::vector<Face>::iterator F=faces.begin();F!=faces.end();F++){
    ...
    else if(F->fissure == 1) { //On eénalise comme si les 2 ddl étaient sur le bord !
      Ep += mu * eta / F->h * F->S * contraction_double( (solide[F->voisins[0]].discrete_gradient) * F-> inertie, (solide[F->voisins[0]].discrete_gradient).T() );
