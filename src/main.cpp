...
- Description g&eacute;om&eacute;trique de chacune des particules face par face: 
 ...
 pr&eacute;sente de l'autre cÃ´t&eacute; de la face. Si aucune particule n'est pr&eacute;sente de l'autre cÃ´t&eacute; 
...
int main(){
  ...
  double rho1,nu1,E1,T1,cfl1,Amort, B1,n1,A1,H1,G1,sig1,t1,penalisation;
  bool rep1, flag2d1;
  param >> s >> rep1 >> s >> numrep1 >> s >> N_dim1 >> s >> flag2d1 >> s >> rho1 >> s >> nu1 >> s >> E1 >> s >> T1 >> s >> cfl1 >> s >> nimp1 >> s >> Nmax1 >> s >> recon >> s >> Amort >> s >> penalisation >> s >> B1 >> s >> n1 >> s >> A1 >> s >> H1 >> s >> G1 >> s >> sig1 >> s >> t1 >> s >> mt >> s >> nom_fichier;
  ...
  const double eta = penalisation; //valeur paramètre pénalisation
...
//En cas de reprise
  ...
  S.eta = eta; //Paramètre de pénalisation vient du fichier param.dat
  ...
  S.Solve_vitesse(pow(10.,-8.), false, 1.0, -0.0000000001, 45.);
  for(std::vector<Particule>::iterator P=S.solide.begin();P!=S.solide.end();P++) {
    P->u_prev = P->u;
    if(P->u.squared_length() != 0.)
