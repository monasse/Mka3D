...
int main(){
  ...
  const double B = B1; //Pour Ècrouissage isotrope de JC
  const double n = n1; //Pour Ècrouissage isotrope de JC
  const double A = A1; //Limite Èlastique initiale
  const double H = H1; //Module d'Ècrouissage linÈaire
...
//En cas de reprise
  ...
  for(std::vector<Face>::iterator F=S.faces.begin();F!=S.faces.end();F++) {
    if(F->fissure) {
      F->Dx[0] = Vector_3(0.,0.,1.) * (F->centre.x() - 1.5) / 1. * 0.1;
      F->Dx[1] = Vector_3(0.,0.,-1.) * (F->centre.x() - 1.5) / 1. * 0.1;
    }
    else if(not(F->fissure)) {
      if((F->centre).x() > 1.5 && (F->centre).y() > 0.) {
	F->I_Dx = Vector_3(0.,0.,1.) * (F->centre.x() - 1.5) / 1. * 0.1;
      }
      else if((F->centre).x() > 1.5 && (F->centre).y() < 0.)
        F->I_Dx = Vector_3(0.,0.,-1.) * (F->centre.x() - 1.5) / 1. * 0.1;
    }
  ...
  for (int n=0; (t<T) && n<Nmax; n++){
    ...
    S.test_fissuration(t); //Test et gËre fissuration s'il y en a
    //Update of time
    t+= dt;
  }
